...
| 27B | Launch Readiness, legal/policy track — Terms of Service, Privacy Policy, age rules, cookie disclosure, adult-owner review | **In progress, not complete.** PR1 (legal-readiness inventory and adult-owner decision packet — see `docs/milestones/MILESTONE_27B_LEGAL_READINESS_SPECIFICATION.md` and `docs/milestones/MILESTONE_27B_ADULT_OWNER_DECISION_PACKET.md`) is documentation/planning only: no legal page has been published, no legal text has been approved, all 22 adult-owner decisions remain unanswered, and public signup remains closed to the general public (invite-gated) pending those decisions. The decision packet is deliberately **not** being handed to the adult owner until the final pre-launch gate — earlier handoff would invite piecemeal answers before the full picture (including 27A's remaining items) is ready for one review pass. Those decisions, and the legal-page drafting/publication PRs that follow them, remain the current launch blocker. |
| Deployment-surface hardening | Cross-cutting infrastructure — production should serve only what the public site needs to run, independent of any one milestone's content | **In progress.** A read-only audit of the deployed Cloudflare Pages surface found `docs/` and `supabase/` served directly and publicly on both production domains (internal architecture/milestone/legal-readiness documents, SQL migration/rollback/test source). Blocked via two narrow, path-scoped Cloudflare Pages Functions (`functions/docs/[[path]].js`, `functions/supabase/[[path]].js`) requiring no Cloudflare Dashboard change — see `docs/DEPLOYMENT.md` §5. `tests/`, `tools/`, `.github/`, `.claude/` were already blocked by a pre-existing Dashboard build-command/WAF layer. No Dashboard, DNS, Supabase, HSTS, or other production setting changed; no legal page, signup behavior, or adult-owner decision touched by this work. Not part of 27A or 27B's own scope — public signup remains invite-gated throughout. |
