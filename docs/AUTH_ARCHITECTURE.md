...
| Missing migration? | **Yes, but a documentation/tracking gap, not a security fix — and the trigger itself is approved as-is, not to be changed.** The trigger/function is real, live, and correctly locked down — but exists nowhere in this project's migration history, the same "untracked but correct" pattern the original `profiles` table and its SELECT/UPDATE policies already had. **Decision (2026-07-27): the baseline migration (capturing the live trigger/function definition verbatim, no behavior change) is deferred until after Track B / immediately after launch — not blocking, not scheduled during Track B itself.** See `docs/milestones/MILESTONE_9_PHASE_9E_ARCHITECTURE.md` for the full proposal. |
