## Unreleased — Milestone 25: Follow Notifications

`set_follow()` (migration `0012`, Milestone 7C) never called `create_notification()` — that migration's own header comment flagged it as explicitly out of scope at the time. This milestone closes the gap: a builder now gets a notification the moment someone new follows them. Migration `0037` widens `notifications_type_check` to add `'follow'` and updates `set_follow()`'s `p_followed = true` branch to capture whether its `insert ... on conflict do nothing` actually inserted a row (via `returning id into v_inserted_id`), calling `create_notification()` only when it did — reusing the exact atomic pattern from `0036`'s report-resolution fix. A duplicate follow click, an idempotent re-call, and unfollowing all correctly notify nobody; unfollowing then following again does notify, since that's a genuinely new row. Notifications store `type='follow'`, `actor_id` = the follower, `build_id`/`comment_id` = null (a follow has no associated build). `notificationFormat.js` renders `"{name} followed you."` linking to the follower's own profile, built from the trusted `actor_id` column rather than any joined, user-controlled text. The rollback is deliberately behavioral rather than a full schema reversal — a required correction made before implementation began: narrowing `notifications_type_check` back once real `'follow'` rows exist would either fail outright or require destroying legitimate notifications, so the rollback restores only `set_follow()`'s pre-0037 body (future follows stop notifying) and never touches the constraint or any existing `'follow'` row. Verified directly, not assumed: rolled back locally with three real follow notifications already present, confirmed all three were untouched and a new follow created no fourth row, then restored forward again. Live verification also surfaced and fixed a pre-existing, unrelated bug in `notificationRepository.js`'s `enrichNotifications()` — an unguarded `.in("id", ids)` call that breaks entirely (Postgres `22P02`) whenever any notification in a batch has a null `build_id`, which already affects `report_resolved` notifications live in production today. Not yet applied to production.

## Unreleased — Milestone 24: Moderator Report Queue

Closes the trust-and-safety gap opened in Milestone 22: builders could submit content reports via `ReportButton.js`, but `resolve_report()` (migration `0028`) had zero callers anywhere in the codebase — moderators had no way to see or act on what had been reported. No migration needed; the existing `content_reports`/`moderation_actions` schema and `resolve_report()` RPC already supported everything this milestone required. New moderator/staff-only page, `pages/moderation.html` ("Reports"), gated client-side by the same `getProfileRoles()` pattern `renderProfile.js` already used, with RLS as the real authorization boundary underneath (`is_platform_moderator(auth.uid())`) — the page fails closed (both content and access-denied markup ship `hidden` in the raw HTML) rather than briefly rendering protected data while the role check resolves. Open reports show a human-readable target (never a raw UUID as the primary label), reason, reporter, and two resolution actions — "No violation" (maps to the existing `dismissed` status) and "Violation confirmed" (maps to the existing `reviewed` status) — each behind an explicit confirmation dialog stating exactly what will happen. Resolving a report records the decision only; it never unpublishes, removes, or otherwise acts on the reported content, and the UI says so directly. A read-only, newest-first Resolved history tab reuses the same data. Target context (build/comment/profile) is resolved via batched lookups grouped by type, not a per-row query; a target that no longer exists (deleted, unpublished, or made private) renders a clear "unavailable" state instead of crashing. Fixed two related latent bugs surfaced by giving `resolve_report()` its first real caller: `notificationFormat.js` had no case for the `report_resolved` notification type the RPC has always sent (fell through to a broken default that built a dead link), and `.badge[hidden]` had no `display: none` override (the same hidden-attribute/class-display specificity trap fixed elsewhere in this codebase), both now fixed. `resolve_report()` originally matched by report id alone with no guard against re-resolving an already-resolved report, mitigated only with a client-side pre-resolve check — see the follow-up entry directly below for why that turned out to be insufficient and how it was actually closed.

## Unreleased — Milestone 24 follow-up: atomic guard against double-resolving a report

PR review for the above milestone empirically demonstrated (two real, separately-authenticated database sessions calling `resolve_report()` against the same open report in sequence, not just reasoned about) that the original client-side-only mitigation had a genuine time-of-check-to-time-of-use gap: both calls returned success, the second silently overwrote the first's decision, and left two `moderation_actions` rows plus two reporter notifications for what should have been one resolution. Closed by `supabase/migrations/0036_resolve_report_atomic_status_guard.sql` — `resolve_report()` keeps its exact signature, security mode, authorization check, and side effects, but its `UPDATE` now adds `and status = 'open'` to the `WHERE` clause and uses `UPDATE ... RETURNING` itself as the atomic claim (ordinary Postgres row-level locking under READ COMMITTED, no new isolation level needed). A report that no longer matches now raises a distinct `'This report has already been resolved.'`, separate from the pre-existing `'Report not found.'` case, so `renderModerationPage.js` can give honest feedback and reconcile both the Open and Resolved views against the server rather than guessing locally. The existing client-side freshness check remains as a fast path only, explicitly documented as not the concurrency boundary. Re-running the exact two-session demonstration after the fix confirms the second call is now rejected and exactly one audit row/notification exists.
