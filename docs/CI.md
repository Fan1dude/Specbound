...
Both are fast, dependency-free, and run directly against a checked-out repo with no secrets and no network access beyond `actions/checkout`/`actions/setup-node`.

**Known limitation:** both checks are regex-based, not a real HTML/CSS/JS parser. They won't catch a reference built at runtime from a variable (e.g. a template-literal-constructed asset path) — those are outside static analysis by construction and need manual/browser verification instead.
