...
**Intended to be published:** `index.html`, `404.html`, `robots.txt`, `sitemap.xml`, `manifest.webmanifest`, `_headers`, `pages/**`, `css/**`, `js/**`, `assets/**`, `design-system.html`.
...
**Blocked by this repository's own Cloudflare Pages Functions:** `docs/` (internal architecture/milestone/legal-readiness documents) and `supabase/` (SQL migration/rollback/test source, and the `product-metadata` Edge Function's own source). This supersedes an earlier decision in this same document that `supabase/**` was fine to publish because the SQL files aren't executable by a static host and don't themselves grant database access — both still true — but a deployment-surface audit (Milestone: deployment-surface hardening) tightened the policy to "production contains only what the public site needs to run," under which schema/RLS/function source is internal material regardless of whether it's exploitable, and `docs/` was found exposed the same way with no prior decision covering it at all (a documentation gap, not a prior deliberate choice).

`functions/docs/[[path]].js` and `functions/supabase/[[path]].js` are auto-detected by Cloudflare Pages from the `/functions` directory — no Cloudflare Dashboard setting is required or was changed. Each function is scoped to exactly one path prefix (Cloudflare's file-based Functions routing: a function under `functions/docs/` is only ever invoked for requests under `/docs/*`) and returns a plain `404` with no request body content. Because they're scoped, not root-level, they have no effect on any other route — in particular they do **not** intercept `pages/**`, `css/**`, `js/**`, or `/`, so `_headers`' CSP/HSTS continues to apply to every real page exactly as before (Cloudflare Pages Functions responses are documented as not receiving `_headers`-applied headers, which only matters for the two blocked prefixes themselves, not for anything else).

Verified live via a commit-pinned Cloudflare preview deployment: required pages/assets return `200`; `/docs/*` and `/supabase/*` return a genuine `404` with a plain "Not Found" body (not a 200 redirect to the homepage, and not Cloudflare's generic block page); the site's own custom `404.html` still renders for a genuinely nonexistent path; CSP/HSTS headers are unchanged on the homepage.

`tools/ci/check-deployment-surface.js` (wired into CI as a ninth static check) computes the effective production file set — every git-tracked file minus `docs/`, `supabase/`, `tests/`, `tools/`, `.github/`, `.claude/` — and asserts required runtime files are present, no excluded path is present, every HTML/CSS/JS reference in that computed set still resolves within it, and no secret-shaped pattern appears anywhere in it. It proves itself non-vacuous on every run: before trusting a clean pass, it runs the same audit against a disposable copy with one excluded file deliberately injected and one required file deliberately deleted, and fails loudly if either sabotage isn't caught.

**What would still be needed for a true build-output allowlist** (not implemented, not required to close the exposure found): Cloudflare Pages' "Build output directory" and "Build command" fields are Dashboard-only settings for a Git-integrated project like this one — there's no confirmed, repository-committed way to override them (a `wrangler.toml`'s effect on Git-integration builds, as opposed to CLI/direct-upload deploys, isn't clearly documented as authoritative by Cloudflare, so it wasn't relied on). If an adult owner ever wants a true positive-allowlist build artifact instead of this prefix-blocklist: in the Cloudflare Pages dashboard, Workers & Pages → the Specbound project → Settings → Builds & deployments, change **Build command** to something that copies only the required files into a fresh `dist/` directory (deterministic, starting empty every build) and change **Build output directory** from `/` to `dist`; verify with a preview deployment on a non-`main` branch before it ever touches `main`; rollback is reverting both fields to their current values (`rm -rf tests tools .github .claude` / `/`) plus a redeploy. This is a Dashboard change and was intentionally not made as part of this milestone.
