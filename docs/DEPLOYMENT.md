...
- **`robots.txt`:** disallows `/pages/settings.html`, `/pages/workshop.html`, `/pages/notifications.html`, `/pages/moderation.html`, `/pages/feedback.html`, `/pages/my-feedback.html`, `/pages/login.html`, `/pages/signup.html`, `/pages/forgotPassword.html`, `/pages/updatePassword.html`, `/pages/build/edit.html`, `/design-system.html`, and `/tests/` (all account-specific, private, or developer-only — no public SEO value); references `sitemap.xml` at `https://specboundapp.com/sitemap.xml`. A live fetch today confirmed the repository's own rules are served intact, but Cloudflare additionally injects a "Managed content" block ahead of them — AI-crawler-specific `Disallow` rules (GPTBot, CCBot, Bytespider, etc.) and a `Content-Signal` directive — that isn't present in this repo's committed file. This is Cloudflare account/zone-level behavior, not something `robots.txt`'s source controls.
  - **Policy and its enforcement (added Milestone 27A PR3):** every page carrying `<meta name="robots" content="noindex">` because it requires sign-in to show real content (Feedback, My Feedback, Moderation, Workshop, Settings, Notifications, the draft editor) or is a non-indexable auth-flow utility page (Login, Sign Up, Forgot Password, Update Password) also gets a matching `Disallow` line here, for consistency with the pattern this file already established. Before this PR, `feedback.html` and `my-feedback.html` had the `noindex` meta tag but no `robots.txt` entry — an inconsistency `tools/ci/check-crawl-policy.js` now catches automatically, along with any newly-added page that gates its own content behind `requireAuth()` but isn't yet classified into that check's policy lists. Worth being explicit about the actual SEO mechanism: `Disallow` does not itself guarantee de-indexing — it tells a crawler not to *fetch* a URL, which for pages linked from nowhere outside the signed-in app (true for all of these) also means the crawler never gets far enough to read a `noindex` it would otherwise honor. Specbound accepts that as this policy's deliberate, low-risk tradeoff rather than treating `Disallow` as a substitute for `noindex`.
