...
3. No redeploy of `delete-account` (or `account-deletion-recovery`, which shares this same key) is required — Supabase Functions read secrets at invocation time, not build time.
...
**Second secret, `account-deletion-recovery`'s own `ACCOUNT_DELETION_RECOVERY_SECRET`** (see `docs/DEPLOYMENT.md` §8.2): rotate the same way — regenerate a fresh random value (e.g. `openssl rand -hex 32`), `supabase secrets set ACCOUNT_DELETION_RECOVERY_SECRET=<new value> --project-ref <project-ref>`, entered directly, never through an intermediate file — then update whatever calls this function on a schedule (the Supabase Cron job's stored header value, or an operator's own script) to the new value before the old one is considered retired. Unlike the service-role key, this secret grants no access to anything OTHER than triggering this one recovery worker — a compromise is lower-severity, but still worth treating as real: someone holding it could force extra (harmless, idempotent, bounded-retry) recovery passes, but could not read or modify any data this function's own restricted RPCs don't already expose to `service_role`.

Prior to `delete-account`'s deployment, the original statement remains accurate: this architecture has no server-side secret beyond Supabase's own runtime. If any future function needs its own additional secret, follow this same pattern (a Supabase Function secret, never a repository file, never a Cloudflare Pages environment variable) rather than reinventing one.
...
**A self-service flow has been designed, implemented, and implementation-reviewed, but is NOT yet deployed to production** — migrations `0044`-`0050` (`0049` supersedes `0048`'s original zero-argument deletion RPC after a security review found its reauthentication check insufficient — see `0049_account_deletion_challenge.sql`'s own header; `0050` adds the restricted, service-role-only recovery worker described just below), `supabase/functions/delete-account`, `supabase/functions/account-deletion-recovery`, and `js/pages/settings/renderDeleteAccountSection.js`, per decision packet item 11's requirement that self-service deletion exist before public registration opens (`docs/milestones/MILESTONE_27B_ADULT_OWNER_DECISION_PACKET.md`). See `docs/DEPLOYMENT.md` §8.1/§8.2 for the exact deployment steps and production verification checklists, none of which have been performed yet. This manual procedure remains necessary as a fallback even after that flow deploys — for accounts whose owner has lost access and cannot complete the self-service reauthentication step, and for any case requiring a legal hold (§10's own audit-log durability concern below is exactly what motivated `0045`'s fix, now applied ahead of the self-service flow specifically so it doesn't reintroduce the same audit-loss problem for every ordinary self-deletion).

**Resuming after Auth deletion, once the former user's own JWT no longer works** — a gap an earlier review of this same flow explicitly disclosed as unaddressed: once `auth.admin.deleteUser()` succeeds, `delete-account` cannot be invoked again for that account (there is no longer any valid session to invoke it with), so a Storage-cleanup failure, or an Auth-deletion failure itself, previously had no path to resolution beyond a support operator manually inspecting `account_deletion_jobs`. `0050_account_deletion_recovery.sql` + `supabase/functions/account-deletion-recovery` close this: a restricted, secret-gated worker (never reachable with a normal user session, at any role — see that function's own header for the full security-boundary reasoning) claims unfinished jobs via `FOR UPDATE SKIP LOCKED`, retries Auth deletion idempotently (an already-gone user counts as success, not failure), retries Storage removal one path at a time (preserving whatever genuinely still fails, never re-marking the whole job complete), and moves a job to `'failed'` — the existing, documented "requires manual attention" terminal state — only once its own bounded retry count is exhausted. Scheduling is not yet configured in production (`docs/DEPLOYMENT.md` §8.2 documents both the recommended `pg_cron`/`pg_net` schedule and the manual-invocation fallback) — until it is, this same manual procedure remains the only way to resolve a stuck job.
