...
7. **Verification — done, 2026-08-01.** Not a separate commit — a verification pass over the six commits above; one real defect found here landed as its own small fix commit rather than being folded silently back into an earlier phase. Static checkers actually live at `tools/ci/check-references.js`, `tools/ci/check-a11y-regressions.js`, `tools/ci/check-syntax.js` (JS, run via `run-tests.js`'s Node/Playwright harness) — not the Python names this section originally guessed from memory; corrected here.
   - **No Node/Playwright in this environment**, so `tools/ci/run-tests.js` and the three `check-*.js` scripts could not be executed directly. Instead, `tests/*.test.html` were run for real by serving the repo with the existing `specbound-static` dev server (`.claude/launch.json`) and driving each test file in the Browser pane, reading `window.__testResults` back via injected JS — the same execution engine `run-tests.js` uses, just driven manually. All three Milestone 20 test files pass in full: `profile.test.html` 24/24, `resolveFeaturedBuild.test.html` 7/7, `buildBuilderJourney.test.html` 12/12.
   - **This caught a real bug**: `renderProjectGallery()` referenced `loadMoreEl` inside `rerenderGrid()` before the `const` declaring it had run (temporal dead zone) — every call threw, so Project Gallery never rendered at all. Fixed by hoisting the three `getElementById` lookups above the first `rerenderGrid()` call; see the dedicated fix commit. A second issue turned out to be a flawed test fixture, not a source bug (`resolveFeaturedBuild.test.html`'s Case 6 gave its "ineligible" build both `completed` status and a later `updated_at` than the legitimate one, so it won via the fallback tier — which correctly trusts its precondition that `builds` is pre-filtered to public — instead of exercising the pin-rejection path the case meant to isolate); fixed in the same commit.
   - **Verified live** (Browser pane, real fixture data injected via console against the real `pages/profile.html`/`profile.css`, no mocking of the render layer itself): Featured Project correctly selects the sole `completed` build over two non-completed ones with no pin set; Project Gallery correctly excludes the Featured build and its filter chips correctly reflect only the categories still present in the gallery; Technology Breakdown correctly counts all 3 public builds across 3 categories (including the one shown as Featured); Builder Journey renders passed-in events; avatar renders at `border-radius: 20px` (`--radius-xl`, the approved deviation); Overview stats sum views/counts correctly. Mobile (375px) and tablet (768px) both confirmed via computed styles: Hero stacks to `column` on mobile and stays `row` on tablet, Featured Project's image aspect ratio switches to 16:9 on mobile, the Technology Breakdown legend goes to 2 columns at both mobile and tablet widths, the gallery filter row becomes horizontally scrollable on mobile, and neither width produces horizontal page overflow (`document.body.scrollWidth` never exceeded `window.innerWidth`).
   - **`renderProfileError()` verified against the real (signed-out, no test credentials used) Supabase backend**: navigating to `pages/profile.html?user=test` hits a real `getPublicProfile` failure and correctly falls into the error state — visible "Profile unavailable" heading, the Headline slot repurposed for the error explanation, and all six conditional sections (`profileEmptyState`/`profileFeatured`/`profileGallery`/`profileTechBreakdown`/`profileJourney`/`profileAbout`) confirmed `hidden` via computed `display: none`, not just the DOM's `hidden` reflected attribute. `pages/settings.html` correctly redirected to `login.html` when signed out, confirming `requireAuth()` still gates the page; the Settings form itself (headline counter, Featured Build picker population) could not be interactively verified without a signed-in session, per the standing no-test-credentials constraint — covered by code review only.
   - **Known, expected, temporary regression — not a code defect**: migration `0024` has not been applied to any real database yet (per standing instruction — schema ships as a migration file only, never applied directly from this environment). Until it is, `profileRepository.js`'s `PUBLIC_PROFILE_COLUMNS` explicitly naming `headline`/`featured_build_id` means **every** live call to `getPublicProfile`/`getProfilesByIds` — not just on the profile page, but anywhere a build's owning profile is attached (Home's Featured section, Explore, individual build pages) — will fail with `column profiles.headline does not exist` until `0024` is applied. Each call site already fails soft or shows a proper error state (confirmed above for the profile page), so nothing crashes outright, but creator attribution/profile links will degrade across the site in the meantime. Apply `0024` to a dev database first via the repo's standing migration procedure, then to production, promptly after this branch merges.
   - **Not run**: axe-core (no axe-core instance available in this environment to attach), and the three static JS checkers listed above (no Node). Recommend running `npm install && node tools/ci/run-tests.js` and the three `tools/ci/check-*.js` scripts, plus a real axe-core pass, before merging — this verification pass is a substitute done under this environment's constraints, not a replacement for the repo's normal CI-equivalent process.
