...
**Why it matters**: Specbound currently uses **no cookies at all**, and the only "tracking" that exists is a single anonymous, random identifier stored in the visitor's own browser purely to stop the same visitor from inflating a project's view count — it's never sent anywhere else and isn't tied to any account (see specification §3/§11). Cloudflare, the company that hosts Specbound, separately adds its own analytics and bot-detection scripts automatically — that's not something this codebase controls or can turn off from within the code.

**Current product behavior**: zero first-party cookies; one anonymous localStorage identifier (view-count dedup only); the browser's normal sign-in session token (also not a cookie, stored the way the login library does it by default); Cloudflare's own edge-injected analytics and bot-management scripts, independent of anything in this repository.

**Options to consider**: keep the current minimal footprint and note it plainly in a future Privacy Policy (simplest — there's genuinely very little to disclose); add first-party analytics later if you want usage insight beyond what Cloudflare already provides, and revisit this decision then; decide whether Cloudflare's edge-injected scripts need their own disclosure line in a future Privacy Policy even though they're not something the app code controls.
