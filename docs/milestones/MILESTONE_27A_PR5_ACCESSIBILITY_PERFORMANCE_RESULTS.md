...
| Heading hierarchy | ~~**Informational, not fixed this pass:** every page has two `<h1>` elements — the navbar's logo (`<h1 class="logo">`, `js/core/layout.js:117`, injected on every page via `loadNavbar()`) and the page's own content `<h1>` (e.g. Home's "Document Every Build."). Confirmed live on production (`document.querySelectorAll("h1")` returned 2 elements on the homepage). Fixing this means changing the shared navbar component's semantics sitewide — broader than this pass's narrow-fix policy allows; flagged as follow-up work, not fixed here.~~ **Fixed by the sitewide `a11y-heading-semantics` follow-up:** the navbar brand (`js/core/layout.js`'s `loadNavbar()`) now renders as a plain `<div class="logo">`, never a heading, so every complete page has exactly one meaningful `<h1>`. `design-system.html`'s typography-preview sample (formerly a second real `<h1>`) was corrected the same way, in a follow-up correction pass — it is not treated as an exception, and is covered by the same generic one-`<h1>`-per-page check as every other page. Covered by a dedicated regression test (`tests/headingSemantics.test.html`), non-vacuous by construction; see §11.4 for exactly what that test does and does not verify. |
...
1. ~~Every page has two `<h1>` elements (navbar logo + page content) — sitewide navbar semantics change, broader than this pass. **Still open, tracked as a separate sitewide follow-up — not addressed by the PR #27 review follow-up (§11).**~~ **Fixed by the `a11y-heading-semantics` follow-up** — see the Heading hierarchy row in §5 and §11.4.
...
The duplicate-`<h1>` finding (§5, §7 informational item 1 — navbar logo `<h1>` + page content `<h1>` on every page) was left open, unfixed, and out of scope for this PR by design — it required a sitewide navbar semantics change. **Fixed separately by the narrowly-scoped `a11y-heading-semantics` follow-up**, which changed only the shared navbar brand element (see the Heading hierarchy row in §5), and by a subsequent correction pass on the same PR that also fixed `design-system.html`'s typography-preview sample and replaced the regression test's hardcoded page list with automatic discovery of every git-tracked complete page.

What that follow-up's verification actually establishes, precisely:
- **Guaranteed by automated source/DOM checks (`tests/headingSemantics.test.html`), run on every CI push:** every git-tracked root-level and `pages/*.html` document resolves to exactly one visible `<h1>` in its raw or (for two explicitly-registered auth-gated pages) dynamically-rendered state; the navbar carries no heading semantics on any page; a newly added page with zero or multiple `<h1>`s fails the run with its file path printed, rather than passing silently.
- **Verified live in a browser during this and the correction pass** (signed-out, no backend required): Home, Login, Signup, Explore, a category page, 404, and `design-system.html` — axe-core clean, exactly one visible `<h1>`, unchanged navbar geometry/keyboard order/link destination/accessible name, no horizontal overflow at desktop/tablet/mobile/200%-zoom-equivalent widths.
- **Not verified live, and not claimed to be:** the real loaded/not-found states of `pages/build/build.html` and `pages/profile.html` (no reachable backend in this environment to populate them — the loading skeleton's single `<h1>` was confirmed, but the eventual success/error content swap was only confirmed by source inspection, not by watching it render), and the real signed-in Feedback/Moderation "allowed" states in production (the regression test exercises this state with a mocked role check, not a live authenticated session). These remain genuinely unavailable without production test data or an authenticated session, and should not be read as covered by this document's "Fixed" language above.
