...
The duplicate-`<h1>` finding (§5, §7 informational item 1 — navbar logo `<h1>` + page content `<h1>` on every page) was left open, unfixed, and out of scope for this PR by design — it required a sitewide navbar semantics change. **Fixed separately by the narrowly-scoped `a11y-heading-semantics` follow-up**, which changed only the shared navbar brand element (see the Heading hierarchy row in §5).
