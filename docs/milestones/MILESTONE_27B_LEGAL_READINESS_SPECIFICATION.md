...
- Cloudflare — "Cloudflare Cookies" (confirms `cf_clearance` is used for JavaScript detections, is set with `SameSite=None; Secure; Partitioned`, and "can also operate as a first-party cookie on the primary domain"): https://developers.cloudflare.com/fundamentals/reference/policies-compliances/cloudflare-cookies/
