...
| Cookie Policy | *(does not exist as a distinct page)* | Not applicable yet — no cookies are used in the product at all (§3), so no cookie-specific disclosure has been built. If any future feature introduces cookies, or if the adult owner decides analytics/local-storage usage should be disclosed regardless, this would need to be created or folded into the Privacy Policy. | — |
...
Fully covered in §3 and the data-category matrix (§4). Summary for policy-drafting convenience: **zero first-party cookies**; Supabase's Auth session token and one anonymous view-tracking UUID live in `localStorage`; two more `localStorage`/`sessionStorage` keys exist for draft-autosave and onboarding-dismiss state, neither containing personal data. Cloudflare independently injects a Web Analytics beacon at the edge (outside this repo's control) and, on at least the custom domain, a Bot Management detection script — both are Cloudflare account/zone-level, not something this codebase configures or can disable through code changes alone. No third-party advertising or tracking-pixel cookie/script of any kind exists.
