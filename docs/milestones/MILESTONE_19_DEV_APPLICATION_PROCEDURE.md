...
- **A Supabase project with migrations `0000`–`0019` already applied.** This app's own convention (`supabase/migrations.md`) is manual application via the Supabase SQL editor — there's no CLI/CI pipeline. `0000_baseline_pre_tracked_tables.sql` is not optional on a fresh project — it creates `profiles`, `builds`, and `build_revisions`, which predate migration tracking and are not created by any other tracked migration (see `docs/DATABASE.md`'s Known Gap section; this was the exact failure a from-empty-database dry run hit on 2026-08-01, `0002` erroring with `relation "public.builds" does not exist"`). Confirm readiness by checking that `public.components` does **not** yet exist (`select to_regclass('public.components');` should return `NULL`) and that `public.builds` **does** (should return `'public.builds'`, not `NULL`).
...
- **Know which project you're pointing at.** `js/core/config.js` currently points to `xpxjqyraizntbtijzoyp.supabase.co`. Live-verified in this session, that project already has real user-generated content (real accounts, real builds) — its `profiles`/`builds`/`build_revisions` tables already exist there and were never created by `0000` (that migration is additive-`CREATE TABLE`-only, so running it against a project where these tables already exist will simply fail with "relation already exists," not overwrite anything — but there's no reason to run it there at all). The migrations themselves are purely additive (new tables only — nothing here alters or drops an existing table), and the test file is written to roll back everything it does, but neither of those is the same as "verified safe in practice" in this environment. **If a disposable/staging Supabase project is available, prefer it for the test-file run and the rollback drill (§3, §5); use the real project only for the final forward-apply once those have passed elsewhere.** If a separate project isn't practical, the procedure below is still safe to run against the shared one (skipping `0000`, which it doesn't need) — just be deliberate about the two test accounts in §6 (clearly-labeled, disposable, easy to tell apart from real users in Explore/search afterward).
...
- **Two throwaway accounts you're prepared to create** through the app's normal signup flow for §6 — this procedure doesn't touch `auth.users` directly for the browser portion (only the SQL test file does, and only inside a rolled-back transaction). On a `0000`-bootstrapped fresh project, signup now correctly creates a matching `profiles` row (the same signup trigger the real project has, reconstructed — see `0000`'s own header for exactly what evidence that reconstruction is based on).
