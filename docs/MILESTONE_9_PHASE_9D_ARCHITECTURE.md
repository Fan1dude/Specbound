...
**Status: Implemented and verified (2026-07-27), with corrections made during implementation — see [Implementation notes](#implementation-notes) at the end of this document.** Cloudflare Pages approved and used as the hosting platform.
...
Corrections and refinements discovered while implementing this plan, applied and documented rather than silently absorbed:

- **No Cloudflare Pages ignore mechanism exists.** §1.2 originally assumed a host-level exclude/ignore config would keep `tests/`/`.claude/` out of the deploy. Research during implementation confirmed Cloudflare Pages has **no `.pagesignore`-equivalent** for git-connected deploys (a longstanding, documented community-requested-but-unbuilt feature). Implemented instead via a minimal build command (`rm -rf tests .claude`) with the output directory set to repository root — functionally equivalent, fits within "no build step unless Cloudflare requires a minimal command." See `docs/DEPLOYMENT.md` §1.
- **The caching strategy in §3.1 was wrong for Cloudflare Pages specifically, and was corrected.** The original plan recommended long `max-age` values for CSS/JS/assets on the theory that atomic, auto-invalidated deploys make aggressive caching safe. Cloudflare's own documentation explicitly recommends *against* custom `Cache-Control` overrides on a custom domain for exactly this app's shape (no content-hashed filenames) — doing so risks serving stale assets after a deploy, because the custom-domain traffic path applies standard HTTP caching semantics independent of Cloudflare's deployment-aware internal cache invalidation. **Corrected approach, actually implemented**: no `Cache-Control` override at all in `_headers` — rely entirely on Cloudflare Pages' built-in per-deployment cache invalidation, which is already correct and requires no configuration. `_headers` contains only the security headers (CSP + the standard hardening set). See `docs/DEPLOYMENT.md` §8 for the full reasoning.
- **The private/account-gated page list grew during implementation.** §2.1's `robots.txt` draft only listed `settings.html`/`dashboard.html`/`login.html`/`signup.html`. Implementation found `workshop.html` and `notifications.html` are equally auth-gated (confirmed via live redirect-to-login behavior), and `pages/build/edit.html` is both auth-gated and per-draft-dynamic — all three were added to `robots.txt`'s disallow list and given `<meta name="robots" content="noindex">`, for consistency with the original four.
- **A CSP correctness question was resolved rather than assumed.** §3.5 flagged a real open question: does `style-src` (with no `'unsafe-inline'`) block this app's several `element.style.property = value` / `.style.setProperty()` calls (the progress bar fill, mobile-nav scroll lock, image-reveal toggle)? Verified against current CSP spec behavior: `style-src` restricts the HTML `style` attribute and `.cssText`, but explicitly does **not** restrict direct CSSOM property assignment or `.setProperty()` — confirmed this app uses neither of the restricted patterns anywhere. The shipped CSP has no `'unsafe-inline'` for `style-src` and was verified live (temporarily sending the real CSP header from the dev server, browsing the full app surface, confirming zero violations) before being finalized in `_headers`.
- **The OG image is a real rendered PNG, not a placeholder or an SVG.** §2.5 didn't specify a production method — implemented via Pillow (Python), rendered at the standard 1200×630 OG size, matching the app's actual brand colors and the Phase 9C favicon's visual language. SVG was deliberately avoided for `og:image` specifically since Twitter/Facebook/Discord link-preview crawlers don't reliably support SVG for this purpose (a JPG/PNG is required for consistent rendering across platforms) — noted as a real technical constraint, not a stylistic choice.
- Everything else (hosting platform, deployment pipeline, Supabase config, branch strategy, `robots.txt`/`sitemap.xml` content, favicon PNG fallback, error-page decision, verification methodology) was implemented as originally proposed.

Full implementation detail lives in `docs/DEPLOYMENT.md` and `docs/OPERATIONS.md` — this document remains the historical record of what was proposed and reviewed.
