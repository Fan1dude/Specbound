-- Rollback for: 0006_unpublish
--
-- Drops set_build_visibility() entirely, and restores publish_draft() to
-- its exact pre-0006 definition (from 0005_revision_history_and_restore.sql
-- — republishing no longer restores visibility='public'). Any build that
-- was unpublished-then-republished while 0006 was live keeps whatever
-- visibility it currently has; this rollback does not change existing
-- data, only future behavior.

begin;

drop function if exists public.set_build_visibility(uuid, text);
